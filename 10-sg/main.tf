module "db" {
  ...
  sg_discription = "SG for DB MySQL Instances"
...
module "ingress" {
  ...
  project_name = var.project_name
  environment = var.environment
  sg_discription = "SG for Ingress controller"
  vpc_id = data.aws_ssm_parameter.vpc_id.value
  common_tags = var.common_tags
  sg_name = "ingress" 
}
...
module "cluster" {
  ...
  project_name = var.project_name
  environment = var.environment
  sg_discription = "SG for EKS Control plane"
  vpc_id = data.aws_ssm_parameter.vpc_id.value
  common_tags = var.common_tags
...
module "node" {
  ...
  project_name = var.project_name
  environment = var.environment
  sg_discription = "SG for EKS node"
  vpc_id = data.aws_ssm_parameter.vpc_id.value
  common_tags = var.common_tags
  sg_name = "eks-node" 
}
...
module "bastion" {
  ...
  project_name = var.project_name
  environment = var.environment
  sg_discription = "SG for DB Bastion Instances"
  vpc_id = data.aws_ssm_parameter.vpc_id.value
  common_tags = var.common_tags
  sg_name = "bastion" 
}
...
module "vpn" {
  ...
  project_name = var.project_name
  environment = var.environment
  sg_discription = "SG for VPN Instances"
  vpc_id = data.aws_ssm_parameter.vpc_id.value
  common_tags = var.common_tags
  sg_name = "vpn" 
